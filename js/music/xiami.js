...
var _duration =368.9901;

 /**
 * start_edit :http://localhost:8080/FgzWeb
 * http://115.238.227.158:8080/fgzweb 
 */
...
// http://115.238.227.158:8080/fgzweb/baiduMp3/c70a61fbebf656cdafe7b73b41f866fe.mp3
...
$(function(){   
	swfobject.embedSWF("swf/dewplayer.swf", "dewplayer_content", "1", "1", "9.0.0", false, flashvars, params, attributes);
 }); 
 
function getMp3Audio(){
	return document.getElementById("myaudio");
}

function play() {
  var dewp = document.getElementById("myaudio");
...
function setMp3(mp3filepath) {
  var dewp = document.getElementById("myaudio"); 
  if(dewp!=null) {
  	console.log("mp3filepath:="+mp3filepath);
	dewp.dewset(mp3filepath);
  }
}
function go(index) {
  var dewp = document.getElementById("myaudio");
...
//	 var d = Number(val.toFixed(2));
//	 alert(dewp.dewvolume(d));
//}
//}
...
//	  /*显示歌曲总长度*/ 
	...
	$(".start em").click(function(){
		...
		/*播放歌词*/
//	    var lrcurl = $(this).data("lrcurl"); 
//		console.log("lrcurl：="+lrcurl) ;
//		if (lrcurl){
//			getReady(lrcurl);//准备播放 lrcurl:歌词的URL地址
//		} 
		...
		/*换右侧图片*/ 
		if (imgurl.indexOf("hiphotos")==-1) { //解决百度图片盗用问题
        ...
        $.getJSON(agentUrl+'/mp3AgentServer/',{'song_id' : songid},  
		   function (json) {  
		   	    console.log("--111--");
   			    setMp3(playUrl+json.mp3path) ;     			     
				getReady(null,json.song_lrc_info);//准备播放 lrcurl:歌词的URL地址 
...
function updateProgress(){
	...
	/*显示歌曲当前时间*/
	var curTime = calcTime(Math.floor(getpos()));
	$(".position").html(curTime);
	/*进度条*/
	var lef = 678*(Math.floor(getpos())/Math.floor(_duration));
...
function getReady(lrcurl,song_lrc_info)//在显示歌词前做好准备工作 
{ 	 
	 if (song_lrc_info){
	 	showLrcInfo(song_lrc_info);
	 } else {
	    $.ajax({
			type: 'get',
			url: lrcurl, 
			...
			error: function (XMLHttpRequest, textStatus, errorThrown) {
				console.log("请求对象XMLHttpRequest: "+XMLHttpRequest.responseText+" ,错误类型textStatus: "+textStatus+",异常对象errorThrown: "+errorThrown);
			}
		}); 
	 } 
