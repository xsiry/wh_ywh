(function($){  
    ...
    var _subTableName = "rlwindow_page"; 
    
    var _ModeId = "rlwindow_Page_ModelId" ;
    
    var $root = $(".ab_rlwindow_page_clsf");   
...
//	        }	       
	    ...
	    $root.on("click", '.bann_page_eidtcls', function(actionobj) {
		 	var rowobj = $(this);   
		 	_selectNewImg = false ;
		 	var rlw_id = rowobj.data("rlw_id") ;  
		 	var apid =  rowobj.data("apid") ;  
		 	show_DetailModel(rlw_id,apid); 
			actionobj.preventDefault();
			rowobj = null ; 
	    }) ; 
	     //删除
	    ...
	    $root.on("click", '.bann_page_upcls', function(actionobj) {
		 	...
		 	var apid =  rowobj.data("apid") ; 
		 	var actionparam = {}
            actionparam.datajson = JSON.stringify({"rlw_id":rlw_id}) ;
	...
	var f_showdetail= function (row, detailPanel,callback)
        { 
            $(detailPanel).append('<div class="btn-view"><a href="javascript:void(0);" data-apid="'+row.ap_id+'"  class="btn btn-default bann_page_detail_add"><i class="fa fa-gear"></i>&nbsp;&nbsp;创建新右下角升窗</a><span style="  color: red; font-size: 15px; line-height: 20px;margin-left: 10px;">注意：所有广告的图片长宽规格为：高->230像素;宽->280像素; </span></div>');  
...
//	        document.createElement('div'); 
            ...
            $(detailGrid).ligerGrid({
                columns: [
//                          { display: '广告ID', name: 'rlw_id'   },  
			                { display: '广告名称', width:150,name: 'rlw_name'},
			                { display: '显示标题', width:150,name: 'rlw_title'},
			                { display: '点击地址',  width:360, name: 'rlw_url'},
			                { display: '图片', width:100, name: 'rlw_img',render:function (rowdata, rowindex, value){
								 return ' <img class="ad_row_img" src="'+value+'"/>';
							  }     
			                },
			                { display: '运行', name: 'rlw_run' ,width:50,  render:function (rowdata, rowindex, value){
			                	if (value=='on')  
			                	  return "是";
			                	else return "否";
			                  }
			                ...
			                { display: '操作管理', width:160, render: function (rowdata, rowindex, value){
								...
								h += '<a href="javascript:void(0);" class="btn btn-default btn-xs bann_page_eidtcls" data-rlw_id="'+rowdata.rlw_id+'" data-apid="'+rowdata.ap_id+'"><i class="fa fa-edit"></i>修改</a>';
		                        h += '<a href="javascript:void(0);" class="btn btn-danger btn-xs bann_page_deletecls" data-rlw_id="'+rowdata.rlw_id+'" data-apid="'+rowdata.ap_id+'"><i class="fa fa-trash-o"></i>删除</a>';	
		                        h += '<a href="javascript:void(0);" class="btn btn-default btn-xs bann_page_upcls" data-rlw_id="'+rowdata.rlw_id+'" data-apid="'+rowdata.ap_id+'"><i class="fa fa-arrow-up"></i>上移</a>';
		                        h += '<a href="javascript:void(0);" class="btn btn-default btn-xs bann_page_downcls" data-rlw_id="'+rowdata.rlw_id+'" data-apid="'+rowdata.ap_id+'"><i class="fa fa-arrow-down"></i>下移</a> </div>'; 
								return h;
							  } 							    
			                }
                         ], 
			           ...
			           pageSizeOptions:[ 30,50,100], sortName: 'rlw_order',sortOrder:'ASC', 
    ...
    var show_DetailModel = function(rlw_id,ap_id){
    	addNewWindowsForm();
    	console.log("rlw_id:="+rlw_id);
	    var modelForm = $('#'+_ModeId).ligerForm({
			inputWidth: 160, labelWidth: 100, space: 20,
		    validate : true,labelAlign: "right", width :"100%",
			fields:[ 
			        {name:"rlw_id",type:"hidden"}, //主键 
			        {name:"ap_id",type:"hidden"} ,  //外键
		        	 //广告ID 
		        	sub_dict_name_rowParam ,  
		        	{display:"广告名称",name:"rlw_name",validate:{required:true},newline:true,type:"text",attr:{id:"rlw_name_cid"}}, 
		        	{display:"显示标题",name:"rlw_title",validate:{required:true},newline:false,type:"text",attr:{id:"rlw_title_cid"}},
		        	{display:"点击地址",name:"rlw_url",validate:{required:true},width:420,newline:true,type:"text"}, 
                    {name:"rlw_img",type:"hidden",attr:{id:"rlw_img_id"}}, 
		        	{htmltx:'<div  style="height: 200px"><div style="text-align: right;     width: 100px;float:left;">图片：</div><input id="rlw_img_ftp"  type="file" multiple class="file-loading" data-max-file-count="1" data-min-file-count="1"> </div>'},
		        	{display:"是否运行游戏",name:"rlw_run",newline:true,width:100,type:"checkbox"},
		        	sub_dict_game_rowParam 
				]
			,onAfterSetFields:function(){ 
				setFtpAction();
				$("#rlw_img_ftp").fileinput('refresh', { 
		                initialPreview: []
		        }); 
			}
		 });  
		 ...
		 {	  
		    $("#rlw_id").val(rlw_id);
		    $.getJSON('queryTableList/?source='+_subTableName,{"sourceid" : rlw_id},  
			   function (json) {   
...
//		                uploadExtraData: { id: id },
		                initialPreview: [ //预览图片的设置
		                    "<img src='" + json.rlw_img + "' class='file-preview-image'>",
		                ]
		           }); 
			   }  
			); 
...
:"1","success":true,"successno":0}
		   	  ...
		   	  if (successResult.success) {  
	            	 console.log("msg:="+successResult.msg);
	            	  $("#rlw_img_id").val(successResult.msg); 
		              saveFrmData();
		   	  } else { 
    ...
    var setRowParam = function(dict_game_rowParam ,dict_name_rowParam){
		...
		{ 
		   $("#rlw_name_cid").val(rowdata.dict_name);
		   $("#rlw_title_cid").val(rowdata.dict_name); 
		}  
    }
    
    ab_rlwindow_page_c = function(){   
    	ab_page_Init("rlwindow",_subTableName,_modelTitle,f_showdetail,setRowParam);  
		pageBindingAction(); 
		pageSetUp();   
	}
